...
@section('pageSubtitle', 'Manage the images shown on the public gallery page')
...
@section('content')
    <div data-admin-page class="space-y-6">
        ...
        <div class="rounded-xl border border-blue-100 bg-blue-50 px-5 py-4 text-sm text-blue-800">
            ...
            Images you add here appear on the public gallery page. When you have images in the database, only your
            images are shown — the default seed content is hidden.
        </div>
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
            ...
            <div class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
                <div class="flex items-center justify-between">
                    <p class="text-sm font-medium text-slate-500">Sort Order Filled</p>
                    <span class="flex h-9 w-9 items-center justify-center rounded-lg bg-purple-50 text-purple-600">
                        <i class="fa-solid fa-arrow-up-wide-short"></i>
                    </span>
                </div>
                <p id="statSorted" class="mt-2 text-2xl font-extrabold text-slate-900">—</p>
            </div>
        </div>
        ...
        <div class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
            <div class="flex flex-col gap-3 border-b border-slate-200 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
                <h2 class="text-sm font-bold text-slate-900">Your Gallery Images</h2>
            ...
            <div id="loadingState" class="flex flex-col items-center justify-center py-16">
                ...
                <p class="mt-3 text-sm text-slate-500">Loading gallery images...</p>
            </div>
            ...
            <div id="emptyState" class="hidden flex flex-col items-center justify-center py-16">
                ...
                <p class="mt-3 text-sm font-medium text-slate-500">No gallery images added yet</p>
                <button id="emptyAddBtn" type="button"
                    ...
                    <i class="fa-solid fa-plus mr-1"></i> Add Your First Image
                </button>
            </div>
            ...
            <div id="gridWrap" class="hidden p-5">
                <div id="imagesGrid" class="grid gap-5 sm:grid-cols-2 lg:grid-cols-3"></div>
    ...
    <div id="imageModal" class="fixed inset-0 z-50 hidden items-center justify-center p-4">
        ...
        <div class="relative z-10 w-full max-w-xl overflow-hidden rounded-xl bg-white shadow-2xl">
            <div class="flex items-center justify-between border-b border-slate-200 px-5 py-4">
                <div>
                    <h3 id="modalTitle" class="text-sm font-bold text-slate-900">Add Image</h3>
                    <p class="mt-0.5 text-xs text-slate-500">Image will be shown on the public gallery page</p>
                </div>
            ...
            <form id="imageForm" class="max-h-[75vh] overflow-y-auto px-5 py-4">
                ...
                <div class="mt-5 flex items-center justify-end gap-2 border-t border-slate-200 pt-4">
                    ...
                    <button type="submit" id="formSubmitBtn"
                        ...
                        <span id="formBtnText">Save Image</span>
                    </button>
                </div>
            </form>
        </div>
    ...
    (() => {
        ...
        const statSorted = document.getElementById("statSorted");
        ...
        function renderGrid() {
            ...
            imagesGrid.innerHTML = state.data.map((img) => `
                <div class="group flex flex-col overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm hover:shadow-md transition-shadow">
                    <div class="relative h-48 overflow-hidden bg-slate-100">
                        <img src="${escapeHtml(img.image || '')}" alt="Gallery image"
                            class="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105"
                            onerror="this.style.display='none'">
                        <span class="absolute right-2 top-2 rounded-full bg-black/60 px-2.5 py-1 text-[11px] font-semibold text-white">#${escapeHtml(img.sort_order || "—")}</span>
                    </div>
                    <div class="flex flex-1 flex-col p-4">
                        <p class="truncate text-xs font-medium text-slate-500" title="${escapeHtml(img.image)}">${escapeHtml(img.image)}</p>
                        <div class="mt-3 flex items-center gap-1.5 border-t border-slate-100 pt-3">
                            <button type="button" data-action="edit" data-id="${img.id}"
                            ...
                            <button type="button" data-action="delete" data-id="${img.id}"
        ...
        function renderPagination(pagination) {
            ...
            pageInfo.textContent = total === 0 ? "No gallery images" : `Showing ${(current_page - 1) * per_page + 1}–${Math.min(current_page * per_page, total)} of ${total} images`;
        ...
        async function loadImages() {
            ...
            try {
                const listRes = await api(`/gallery?page=${state.page}&per_page=${state.size}`, { signal: controller.signal });
                ...
                if (listRes && listRes.ok) {
                    ...
                    statSorted.textContent = state.data.filter((img) => img.sort_order).length;
                ...
                setLoading(false);
                show(errorState);
                document.getElementById("errorMsg").textContent = "Could not load gallery images. Please try again.";
            } finally {
        ...
        pageSize.addEventListener("change", () => { state.size = Number(pageSize.value); state.page = 1; loadImages(); });
        prevPage.addEventListener("click", () => { if (state.page > 1) { state.page--; loadImages(); } });
        nextPage.addEventListener("click", () => { state.page++; loadImages(); });
        ...
        const openModal = (editMode = false, image = null) => {
            form.reset();
            clearErrors();
            ...
            hide(imagePreview);
            formId.value = "";
            document.getElementById("modalTitle").textContent = editMode ? "Edit Image" : "Add Image";
            formBtnText.textContent = editMode ? "Update Image" : "Save Image";
            imageRequired.style.display = editMode ? "none" : "inline";

            if (editMode && image) {
                formId.value = image.id;
                formSortOrder.value = image.sort_order || "";
                if (image.image) {
                    previewImg.src = image.image;
                    show(imagePreview);
                }
            }
        ...
        addImageBtn.addEventListener("click", () => openModal(false));
        ...
        const fieldEls = { image: formImage };
        const touched = { image: false };
        ...
        function clearErrors() {
            ["errorImage", "errorSortOrder"].forEach(id => {
        ...
        form.addEventListener("submit", async (e) => {
            ...
            try {
                ...
                showToast(data.message || "Gallery image saved.");
                closeModalFn();
                formSubmitBtn.disabled = false;
                setFormSpinner(false);
                formBtnText.textContent = id ? "Update Image" : "Save Image";
                loadImages();
            } catch (err) {
        ...
        imagesGrid.addEventListener("click", async (e) => {
            ...
            if (action === "delete") {
                ...
                if (res && res.ok) {
                    showToast(res.message || "Gallery image deleted.");
                    await loadImages();
                }
            }
        });
