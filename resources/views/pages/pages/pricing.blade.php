...
@section('content')
    ...
    <section class="py-20 bg-white">
        <div class="max-w-356 mx-auto px-4 sm:px-6 lg:px-8">
            <div class="reveal reveal-fade-up text-center mb-16">
                <span class="inline-block text-brand-blue font-bold text-sm uppercase tracking-wider mb-3">Our Pricing
                    Plan</span>
            ...
            <div class="reveal reveal-fade-up grid md:grid-cols-3 gap-8 max-w-5xl mx-auto">
                ...
                @foreach ($plans as $plan)
                    <div
                        class="rounded-2xl p-8 {{ $plan['featured'] ? 'bg-brand-blue text-white ring-4 ring-brand-blue shadow-xl scale-105 relative' : 'bg-brand-light-bg text-heading' }}">
                        @if ($plan['featured'])
                            <span
                                class="absolute -top-4 left-1/2 -translate-x-1/2 bg-brand-secondary text-white text-xs font-bold px-4 py-1.5 rounded-full">Popular</span>
                        @endif
                        <h3 class="text-xl font-bold mb-2 {{ $plan['featured'] ? 'text-white' : 'text-heading' }}">
                            {{ $plan['name'] }}</h3>
                        <div class="mb-6">
                            <span class="text-4xl font-extrabold">{{ $plan['price'] }}</span>
                            <span
                                class="text-sm {{ $plan['featured'] ? 'text-white/70' : 'text-body-muted' }}">{{ $plan['period'] }}</span>
                        </div>
                        <ul class="space-y-3 mb-8">
                            @foreach ($plan['features'] as $feature)
                                <li class="flex items-center gap-3">
                                    <svg class="w-5 h-5 {{ $plan['featured'] ? 'text-white' : 'text-brand-blue' }} shrink-0"
                                        fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M4.5 12.75l6 6 9-13.5" />
                                    </svg>
                                    <span
                                        class="text-sm {{ $plan['featured'] ? 'text-white/90' : 'text-body-muted' }}">{{ $feature }}</span>
                                </li>
                            @endforeach
                        </ul>
                        <a href="{{ route('contact') }}"
                            class="block text-center font-semibold rounded-full px-8 py-3.5 transition-all duration-300 {{ $plan['featured'] ? 'bg-white text-brand-blue hover:bg-white/90' : 'bg-brand-blue text-white hover:bg-brand-hover' }}">
                            Choose Package
                        </a>
                    </div>
                @endforeach
    ...
    <section class="py-16 bg-brand-dark-bg">
        <div class="max-w-356 mx-auto px-4 sm:px-6 lg:px-8 text-center">
    ...
    <section class="py-20 bg-white">
        <div class="max-w-356 mx-auto px-4 sm:px-6 lg:px-8">
            <div class="reveal reveal-fade-up text-center mb-16">
                <span
                    class="inline-block text-brand-blue font-bold text-sm uppercase tracking-wider mb-3">Testimonial</span>
            ...
            <div class="reveal reveal-fade-up grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                @foreach ($testimonials as $t)
                    <div class="bg-brand-light-bg rounded-2xl p-6">
                        ...
                        <p class="text-sm text-body-muted leading-relaxed mb-4 italic">"In today's fast-evolving digital
                            landscape, businesses need a clear IT strategy to align technology with their long-term goals."
                        </p>
