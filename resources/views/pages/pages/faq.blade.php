        ...
        <div class="max-w-356 mx-auto px-4 sm:px-6 lg:px-8">
            <div class="reveal reveal-fade-up grid lg:grid-cols-2 gap-16">
                ...
                <div>
                    ...
                    <p class="text-body-muted text-lg mb-6">If you have an issue or question that requires immediate
                        assistance, you can click the button below to chat live with a Customer Service representative.</p>
                    <p class="text-sm text-body-muted mb-8 italic">Please allow 06 – 12 business days from the time your
                        package arrives back to us for a refund to be issued.</p>

                    {{-- Contact Form --}}
                    <form class="space-y-4" x-data="{ submitting: false }" @submit.prevent="submitting = true">
                        <input type="text" placeholder="First Name"
                            class="w-full px-4 py-3 rounded-xl border border-brand-subtle-border focus:border-brand-blue focus:ring-2 focus:ring-brand-blue/20 outline-none transition-all" />
                        <textarea rows="4" placeholder="Your Message"
                            class="w-full px-4 py-3 rounded-xl border border-brand-subtle-border focus:border-brand-blue focus:ring-2 focus:ring-brand-blue/20 outline-none transition-all resize-none"></textarea>
                        <button type="submit"
                            class="bg-brand-blue hover:bg-brand-hover text-white font-semibold rounded-full px-8 py-3.5 transition-all duration-300">
                            Ask Question Now
                        </button>
                    </form>
                </div>
                    ...
                    @foreach ($faqs as $question => $answer)
                        <div class="bg-brand-light-bg rounded-xl overflow-hidden" x-data="{ open: false }">
                            <button @click="open = !open"
                                ...
                                <svg class="w-5 h-5 text-brand-blue shrink-0 transition-transform duration-300"
