...
@section('content')
    ...
    <section class="relative bg-linear-to-br from-hero-gradient to-white pt-28 pb-20 lg:pt-36 lg:pb-28 overflow-hidden">
        <div
            class="reveal reveal-fade-up max-w-356 mx-auto px-4 sm:px-6 lg:px-8 grid lg:grid-cols-2 gap-16 items-center relative z-10">
            <div class="space-y-6">
                @if (!empty($tagline))
                    <span
                        class="inline-block px-4 py-1.5 bg-brand-blue/10 text-brand-blue text-sm font-semibold rounded-full">{{ $tagline }}</span>
                @endif
                ...
                @if (!empty($heroButtons))
                    <div class="flex flex-wrap gap-4 pt-2">
                        @foreach ($heroButtons as $btn)
                            <a href="{{ $btn['href'] }}"
                                class="inline-flex items-center gap-2 {{ $btn['primary'] ?? false ? 'bg-brand-blue text-white hover:bg-brand-hover' : 'bg-white text-brand-blue border-2 border-brand-blue hover:bg-brand-blue hover:text-white' }} font-semibold px-6 py-3 rounded-xl transition-colors text-sm">
                                {{ $btn['label'] }}
            ...
            @if (!empty($heroImage))
                <div class="flex justify-center lg:justify-end">
                    <img alt="{{ $pageTitle }}" loading="lazy" class="rounded-2xl shadow-lg max-w-md w-full"
                        src="{{ $heroImage }}" />
                </div>
            @endif
        </div>
                    ...
                    <div class="max-w-356 mx-auto px-4 sm:px-6 lg:px-8">
                        ...
                        <div class="grid md:grid-cols-2 lg:grid-cols-{{ $section['cols'] ?? 3 }} gap-8">
                            @foreach ($section['items'] ?? [] as $i => $item)
                                <div class="reveal reveal-fade-up bg-white rounded-2xl p-8 shadow-sm border border-slate-100 hover:shadow-lg transition-shadow group"
                                    style="animation-delay: {{ $i * 0.05 }}s">
                                    @if (!empty($item['icon']))
                                        <div
                                            class="w-14 h-14 bg-brand-blue/10 rounded-xl flex items-center justify-center mb-6 group-hover:bg-brand-blue/20 transition-colors">
                                            <svg class="w-7 h-7 text-brand-blue" xmlns="http://www.w3.org/2000/svg"
                                        ...
                                        <ul class="mt-4 space-y-2">
                                            @foreach ($item['links'] as $link)
                                                <li class="flex items-start gap-2 text-sm text-slate-600">
                                                    <svg class="w-4 h-4 text-brand-blue shrink-0 mt-0.5"
                                    ...
                                    @if (!empty($item['href']))
                                        <a href="{{ $item['href'] }}"
                                            class="inline-flex items-center text-brand-blue font-semibold mt-4 hover:text-brand-hover transition-colors text-sm">
                        ...
                        <div class="grid lg:grid-cols-2 gap-12">
                            @foreach ($section['items'] ?? [] as $i => $item)
                                <div class="reveal reveal-fade-up flex gap-4"
                                    ...
                                    <div
                                        class="w-12 h-12 bg-brand-blue/10 rounded-xl flex items-center justify-center shrink-0">
                                        <svg class="w-6 h-6 text-brand-blue" xmlns="http://www.w3.org/2000/svg"
                                            fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round"
                                                d="M4.5 12.75l6 6 9-13.5" />
                                        </svg>
                        ...
                        <div class="grid lg:grid-cols-2 gap-16 items-center">
                            ...
                            <div class="reveal reveal-fade-up">
                                @if (!empty($section['badge']))
                                    <span
                                        class="inline-block px-4 py-1.5 bg-brand-blue/10 text-brand-blue text-sm font-semibold rounded-full mb-4">{{ $section['badge'] }}</span>
                                @endif
                ...
                <section class="py-20 {{ $section['bg'] ?? 'bg-slate-50' }}">
                    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                        ...
                        <div class="grid sm:grid-cols-2 gap-4">
                            @foreach ($section['items'] ?? [] as $i => $item)
                                <div class="reveal reveal-fade-up flex items-start gap-3 bg-white rounded-xl p-4 shadow-sm border border-slate-100"
                                    style="animation-delay: {{ $i * 0.05 }}s">
                                    <svg class="w-5 h-5 text-brand-blue shrink-0 mt-0.5"
                                        xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                        stroke-width="2" stroke="currentColor">
